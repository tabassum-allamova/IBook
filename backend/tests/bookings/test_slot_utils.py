"""
Unit tests for slot computation pure function.

These tests verify compute_available_slots and _overlaps without any database
or HTTP dependencies. All tests should be GREEN (the pure function exists).
"""
...
class TestOverlaps:
    ...
    def test_adjacent_intervals_no_overlap(self):
        """[9:00, 10:00) and [10:00, 11:00) are adjacent, not overlapping."""
        assert _overlaps(time(9, 0), time(10, 0), time(10, 0), time(11, 0)) is False

    def test_same_interval(self):
        assert _overlaps(time(9, 0), time(10, 0), time(9, 0), time(10, 0)) is True
